...
public class PestoAuthenticationHandler extends AuthenticationHandler{
	...
	public boolean setup(String ssid, KeyShares master, byte[] localKeyShare, Map<Integer, byte[]> remoteShares,
		int id, long allowedTimeDifference, long waitTime, long sessionLength, List<? extends IdPRESTWrapper> serverSetup) {
		return setup(ssid,master,localKeyShare,remoteShares,id,allowedTimeDifference,waitTime,sessionLength,serverSetup,1000L);
	}
	
	public boolean setup(String ssid, KeyShares master, byte[] localKeyShare, Map<Integer, byte[]> remoteShares,
			int id, long allowedTimeDifference, long waitTime, long sessionLength, List<? extends IdPRESTWrapper> serverSetup, long timeoutFactor) {
		this.id = id;
	...
	public OPRFResponse performOPRF(String salt, String username, ECP x, String mfaToken, String mfaType) throws AuthenticationFailedException {
	...
	public boolean validateUsernameAndSignature(String username, byte[] sessionData, long salt, byte[] signature, String operation) {
		...
		if (userKey == null) {
			logger.info("User key is null");
			return false;
		}
		if (!checkSalt(username,salt)) {
			logger.info("Could not validate salt");
			return false;
		}
		byte[] nonce = crypto.constructNonce(username, salt);
