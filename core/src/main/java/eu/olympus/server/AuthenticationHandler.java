...
public abstract class AuthenticationHandler {
  ...
  /**
   * Checks if a user can satisfy a policy and if so, produces a key-value mapping of the claims.
   *
   * Any policy that cannot be satisfied (e.g. the requested attribute does not exist for the user),
   ...
   */
  public Map<String, Attribute> validateAssertions(String username, Policy policy)
      throws PolicyUnfulfilledException {
    ...
    for (Predicate predicate : policy.getPredicates()) {
      ...
      if (operation == Operation.LESSTHANOREQUAL) {
        ...
        if (attribute.getType() == AttributeType.INTEGER) {
          if ((Integer) attribute.getAttr() <= (Integer) predicate.getValue().getAttr()) {
            ...
            throw new PolicyUnfulfilledException(
                "Could not satisfy " + key + " LT " + predicate.getValue().getAttr());
          }
        }
      ...
      if (operation == Operation.EQ) {
        ...
        if (attribute.equals(predicate.getValue())) {
          ...
          throw new PolicyUnfulfilledException(
              "Could not satisfy " + key + " == " + predicate.getValue().getAttr());
        }
      }
      if (operation == Operation.GREATERTHANOREQUAL) {
        ...
        if (attribute.getType() == AttributeType.INTEGER) {
          if ((Integer) attribute.getAttr() >= (Integer) predicate.getValue().getAttr()) {
            ...
            throw new PolicyUnfulfilledException(
                "Could not satisfy " + key + " GT " + predicate.getValue().getAttr());
          }
        }
        if (attribute.getType() == AttributeType.DATE) {
      ...
      if (operation == Operation.INRANGE) {
        ...
        if (attribute.getType() == AttributeType.INTEGER) {
          if ((Integer) attribute.getAttr() >= (Integer) predicate.getValue().getAttr()
              && (Integer) attribute.getAttr() <= (Integer) predicate.getExtraValue().getAttr()) {
            output.put(name, new Attribute(true));
            added = true;
          } else {
            throw new PolicyUnfulfilledException(
                "Could not satisfy " + key + " INRANGE " + predicate.getValue().getAttr() + "-"
                    + predicate.getExtraValue().getAttr());
          }
        }
        if (attribute.getType() == AttributeType.DATE) {
          //In all Date comprobations we use !after/!before instead of before/after because these methods are not inclusive.
          if (!(((Date) attribute.getAttr()).before((Date) predicate.getValue().getAttr())
              || ((Date) attribute.getAttr()).after((Date) predicate.getExtraValue().getAttr()))) {
            output.put(name, new Attribute(true));
            added = true;
          } else {
            throw new PolicyUnfulfilledException(
  ...
  private String getValueAsString(Predicate predicate) {
    if (predicate.getOperation() == Operation.INRANGE) {
      ...
      if (predicate.getValue().getType() == AttributeType.DATE) {
        ...
        result +=
            "-" + DateFormat.getDateInstance(DateFormat.SHORT, Locale.GERMAN).format(dateExtra);
      }
      return result;
    }
  ...
  /**
   * Verifies that AT LEAST ONE of the requestedRoles are granted to the user with the given
   * cookie.
   */
  public void validateSession(String cookie, List<Role> requestedRoles)
  ...
  /**
   * Evaluate the MFA if ANY MFA is activated for the user in question. If so, and if the token is
   * of an active MFA type, evaluate and return the result. If the MFA supplied is of a non-active
   * MFA AND there is another active MFA return false (as this is a malicious attack to circumvent
   * MFA). If no MFA is activated for this user return true as all tokens will be valid in that
   * case. Finally return false if the user does not exist.
   *
  ...
  public boolean conservativeMFAValidation(String username, String token, String type) {
    ...
    if (mayNotAuthenticate(database.getLastMFAAttempt(username), database.getNumberOfFailedMFAAttempts(username),
				mfaAuthenticators.get(type).getTimeoutPeriod())) {
      return false;
    }
